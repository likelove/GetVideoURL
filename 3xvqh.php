...
.w_60 {width:60px;}
 ...
 <body style="text-align:center;background-color:#F5F5F5;"><div style="width:1080px;margin:0 auto;">
  <header>
    <h2 style="color:#800000;">墙外3X在线视频延时下载列表</h2>
  </header>
...
$result = pg_query($dbn, "SELECT xh,host,code,title,pic,mp4 FROM dl3xv where dl is NULL ORDER BY qtime ASC");
if ($result) {
	while ($row = pg_fetch_assoc($result)) {
		...
		echo '<td><input type="button" value="记录" onclick="window.open (\'https://php54-feelsix.rhcloud.com/x3vurl/3xvdc.php?xh='.$row['xh'].'&a=u\');deltr(\''.$row['xh'].'\');"/><p /><input type="button" value="废弃" onclick="window.open (\'https://php54-feelsix.rhcloud.com/x3vurl/3xvdc.php?xh='.$row['xh'].'&a=d\');deltr(\''.$row['xh'].'\');"/></td></tr>';
	}
}
?>
</tbody></table>
<?php
$rs = pg_query($dbn, "SELECT count(*) as rowtotal FROM dl3xv where dl is null");
